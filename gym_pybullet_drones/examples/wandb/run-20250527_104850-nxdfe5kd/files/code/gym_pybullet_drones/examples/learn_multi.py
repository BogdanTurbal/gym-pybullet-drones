#!/usr/bin/env python3
"""
train_multi_improved.py - CORRECTED for matrix observations (n_drones, n_features)
"""
import os
import time
...
# Import your custom extractors (make sure this file is in the same directory)
...
# from MultiTargetAviary import MultiTargetAviary
...
def run(output_folder, gui, record_video, plot, local, wandb_project, wandb_entity, 
    ...
    """
    Main training function with corrected multi-agent architecture
    """
    run_name = f"corrected_multiagent_{extractor_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    expected_shape = (DEFAULT_DRONES, 35)  # 4 drones, 29 features each
    assert obs_shape == expected_shape, f"Expected obs shape {expected_shape}, got {obs_shape}"
    print(f'[INFO] ✓ Observation shape verified: {obs_shape}')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='CORRECTED Multi-agent drone swarm training')
    ...
    parser.add_argument('--wandb_project', default='drone-swarm-corrected', type=str, 
    ...
    print("="*60)
    print("CORRECTED Multi-Agent Drone Swarm Training")
    print("="*60)
    print(f"Extractor type: {args.extractor_type}")
    print(f"Features dim: {args.features_dim}")
    print(f"Observation format: Matrix ({DEFAULT_DRONES}, 29)")
    print("="*60)
