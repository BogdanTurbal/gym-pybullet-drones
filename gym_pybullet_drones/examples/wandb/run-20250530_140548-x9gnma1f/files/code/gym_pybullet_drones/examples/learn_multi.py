...
DEFAULT_FEATURES_DIM = 64  # Slightly smaller for faster training with new reward
...
def run(output_folder, gui, record_video, plot, local, wandb_project, wandb_entity, 
    ...
    paper_reward_hyperparameters = {
        'lambda_1': 200.0,    # Progress reward - increased for faster learning
        'lambda_2': 1.5,     # Perception reward
        'lambda_3': 0.4,     # Perception alignment exponent
        'lambda_4': 0.001,    # Action magnitude penalty
        'lambda_5': 0.001,     # Action smoothness penalty
        #'crash_penalty': 15.0,   # Crash penalty - increased
        #'bounds_penalty': 20.0,   # Out-of-bounds penalty
    }
    ...
    config = {
        ...
        'learning_rate': 3e-4,  # Slightly lower for stability with new reward
        'batch_size': 1024, #// NUM_VEC,  # Larger batch size for stability
        'n_epochs': 5,
    ...
    dummy_env = HoverAviary(obs=DEFAULT_OBS, act=DEFAULT_ACT, ctrl_freq=10)
    ...
    extractor_params = sum(p.numel() for p in model.policy.parameters())#sum(p.numel() for p in model.policy.features_extractor.parameters())
