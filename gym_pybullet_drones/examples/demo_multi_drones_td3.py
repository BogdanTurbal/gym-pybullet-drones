...
Supports TD3, SAC, and PPO models
"""
import os
import time
import argparse
...
DEFAULT_ACT           = ActionType('rpm')  # Changed from 'rpm' to match training
DEFAULT_DRONES        = 1
...
def create_target_sequence(num_drones=4, scale=1.2):
    """Create target sequence matching the training script exactly"""
...
def create_enhanced_plots(metrics_collector, output_folder, episode_num):
    ...
    try:
        ...
        # Plot 8: Action analysis - Updated for multi-algorithm actions
...
def run_demonstration(model_path, output_folder, gui, record_video, plot, num_episodes=1, 
    ...
    # Create target sequence matching training script exactly
    ...
    # Paper-based reward hyperparameters (matching training script)
